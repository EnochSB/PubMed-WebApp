...
class SQLiteArticleRepository:
    ...
    def __init__(self, database_path: str | Path, table_name: str = "articles") -> None:
    ...
    def get_overview(self, top_journal_limit: int) -> OverviewData:
        with closing(self._connect()) as connection:
            ...
            year_rows = connection.execute(
                f"""
                SELECT CAST(pub_year AS INTEGER) AS label, COUNT(*) AS article_count
                FROM {self._quoted_table}
                WHERE pub_year IS NOT NULL AND TRIM(CAST(pub_year AS TEXT)) <> ''
                GROUP BY CAST(pub_year AS INTEGER)
                ORDER BY CAST(pub_year AS INTEGER)
                """
            ).fetchall()
            journal_rows = connection.execute(
                f"""
                SELECT journal AS label, COUNT(*) AS article_count
                FROM {self._quoted_table}
                WHERE TRIM(COALESCE(journal, '')) <> ''
                GROUP BY journal
                ORDER BY article_count DESC, journal COLLATE NOCASE ASC
                LIMIT ?
                """,
                (top_journal_limit,),
            ).fetchall()
    ...
    def get_search_options(self) -> SearchOptions:
        with closing(self._connect()) as connection:
            year_row = connection.execute(
                f"""
                SELECT MIN(CAST(pub_year AS INTEGER)), MAX(CAST(pub_year AS INTEGER))
                FROM {self._quoted_table}
                WHERE pub_year IS NOT NULL AND TRIM(CAST(pub_year AS TEXT)) <> ''
                """
            ).fetchone()
    ...
    def search(self, criteria: ArticleSearchCriteria) -> tuple[Article, ...]:
        ...
        if criteria.start_year is not None:
            conditions.append("CAST(pub_year AS INTEGER) >= ?")
        ...
        sql = f"""
            ...
            FROM {self._quoted_table}
            {where_clause}
            ORDER BY CAST(pub_year AS INTEGER) DESC, title COLLATE NOCASE ASC
        """
